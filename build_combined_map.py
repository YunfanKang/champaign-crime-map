...
<body><div id="app">
 <div id="side">
  ...
  <div class="card"><h2>Plan a route (A → B)</h2>
   <div class="srch" style="margin-bottom:6px"><input id="rStart" placeholder="start, e.g. Market Place Mall"></div>
   <div class="srch"><input id="rEnd" placeholder="end, e.g. Main Quad"><button id="rGo">Route</button></div>
   ...
   <div id="routeInfo" class="small">Enter a start and end to draw the street route and count crimes along it (within the current filters).</div>
  </div>
...
function clearRoute(){routeLayers.forEach(l=>map.removeLayer(l));routeLayers=[];routeLine=null;rInfo.innerHTML='Enter a start and end to draw the street route and count crimes along it (within the current filters).';}
function routeStats(idx){if(!routeLine)return;
 ...
 const VIO=new Set(['Assault','Robbery','Weapons']);let near=0,viol=0;const gc={};
 for(const i of idx){const r=ROWS[i],la=r[2],lo=r[3];
  ...
  if(dm<=corridorM){near++;gc[G[r[0]]]=(gc[G[r[0]]]||0)+1;if(VIO.has(G[r[0]]))viol++;}}
 const yf=365/514;const top=Object.entries(gc).sort((a,b)=>b[1]-a[1]).slice(0,5).map(e=>e[0]+' '+e[1]).join(', ');
 rInfo.innerHTML='<b>'+near.toLocaleString()+'</b> crimes within '+corridorM+' m of the route &middot; <b>'+viol+'</b> violent &middot; ~'+Math.round(near*yf)+'/yr'+(top?'<br><span class="small">'+top+'</span>':'');}
